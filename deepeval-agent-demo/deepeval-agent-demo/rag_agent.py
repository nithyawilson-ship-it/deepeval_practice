...
Architecture:
  - Knowledge base: 9 policy documents stored as embeddings (local HuggingFace model)
  ...
  - LLM: claude-sonnet-4-6 (Anthropic), with retrieval instead of hard-coded data
...
Note on embeddings:
  Anthropic doesn't offer its own embeddings API, so this file uses a free
  local embeddings model (sentence-transformers, via langchain-huggingface)
  instead of OpenAIEmbeddings. No API key or external account needed for this
  part. Anthropic's official recommended embeddings partner is Voyage AI, if
  you'd rather use a hosted model instead of running one locally.
"""
...
# Uses a small local embeddings model — downloads once, then runs offline
